...
=======
 are the Hubbard U and the inverse temperature, respectively. The larger $\beta$
 corresponds to the low temperature regime is considered to depict the ground state of the system. As seen the Kinetic/potential energy decreases/increases with increasing the chemical potential. The lowest total energy is obtained at chemical potential $\mu=0$. 
 The total energy increases by increasing/decreasing the chemical potential, and the larger $\beta$
 (lower temperature), the lower enrgy. The calculations show that the total energy for a relaxed system (at $\mu=0$)
 changes by %10 upon changing $\beta$
 from 3 to 10. Please note that while the calculations are reported for the Hubbard parameters, they can be applied to the Rydberg atom system by appropriate scalling.
>>>>>>> d4cdf37ca354f8b0fc70f3a540aed120292280c7
