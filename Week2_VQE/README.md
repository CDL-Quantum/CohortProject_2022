...
The table above shows the difference between the energy (in units of Hartree) calculated using different methods, for the $H_2$ dimer (with the two $H_2$'s separated by a large distance of 100 $\unicode{x212B}$) vs. twice that of a single $H_2$. Physically, when separation tends to infinity, identical molecules would not affect each other, and the energy of the whole should be the sum of each individual molecule. This exercise is thus a simple test of size consistency, as described above. From this calculation, we see that HF and CCSD reproduce this behavior to a very high accuracy, and are thus ***size-consistent***, whereas CISD is not. 
