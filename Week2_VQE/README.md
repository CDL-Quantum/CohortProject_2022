...
***Cons***: BK trades the locality of occupation number in JW with a partial one (spin orbitals store partial sums of occupation numbers in BK) so that the parity information is not stored completely nonlocal as in JW. As a result, the qubit operators in BK are also considerably more complicated than in JW.
